...
''------------------------------------------------------------------------------------
'' Initialization
dim as const long screenWidth = 800
dim as const long screenHeight = 450

SetConfigFlags(FLAG_WINDOW_RESIZABLE) '' Make the window resizable
InitWindow(screenWidth, screenHeight, "raylib [textures] example - tiled drawing")

'' NOTE: Textures MUST be loaded after Window initialization (OpenGL context is required)
var texPattern = LoadTexture("resources/patterns.png")
...
dim as long MAX_COLORS = (ubound(colors) - lbound(colors) + 1)
...
for i as integer = 0 to MAX_COLORS - 1
  with colorRec(i)
    .x = 2.0f + MARGIN_SIZE + x
    .y = 22.0f + 256.0f + MARGIN_SIZE + y
    .width = COLOR_SIZE * 2.0f
    .height = COLOR_SIZE
  end with
...
do while not WindowShouldClose()    '' Detect window close button or ESC key
  ...
  if IsMouseButtonPressed(MOUSE_BUTTON_LEFT) then
    ...
    for i as integer = 0 to ubound(recPattern)
      if CheckCollisionPointRec(mouse, Rectangle( _
        2 + MARGIN_SIZE + recPattern(i).x, 40 + MARGIN_SIZE + recPattern(i).y, _
  ...
  BeginDrawing()
    ...
    '' Draw options
    DrawRectangle(MARGIN_SIZE, MARGIN_SIZE, OPT_WIDTH - MARGIN_SIZE, _
      GetScreenHeight() - 2*MARGIN_SIZE, ColorAlpha(LIGHTGRAY, 0.5f))

    DrawText("Select Pattern", 2 + MARGIN_SIZE, 30 + MARGIN_SIZE, 10, BLACK)
    DrawTexture(texPattern, 2 + MARGIN_SIZE, 40 + MARGIN_SIZE, BLACK)
    DrawRectangle(2 + MARGIN_SIZE + recPattern(activePattern).x, _
      40 + MARGIN_SIZE + recPattern(activePattern).y, _
      recPattern(activePattern).width, recPattern(activePattern).height, ColorAlpha(DARKBLUE, 0.3f))
