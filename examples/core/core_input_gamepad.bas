...
dim as Texture2D texPs3Pad = LoadTexture("resources/ps3.png")
dim as Texture2D texXboxPad = LoadTexture("resources/xbox.png")

'' Set axis deadzones
const as single leftStickDeadzoneX = 0.1f
const as single leftStickDeadzoneY = 0.1f
const as single rightStickDeadzoneX = 0.1f
const as single rightStickDeadzoneY = 0.1f
const as single leftTriggerDeadzone = -0.9f
const as single rightTriggerDeadzone = -0.9f
...
do while not WindowShouldClose()    '' Detect window close button or ESC key
    ...
    BeginDrawing()
        ...
        if IsKeyPressed(KEY_LEFT) = true and gamepad > 0 then gamepad -= 1
        if IsKeyPressed(KEY_RIGHT) then gamepad += 1

        if IsGamepadAvailable(gamepad) then
            DrawText(TextFormat("GP%d: %s", gamepad, GetGamepadName(gamepad)), 10, 10, 10, BLACK)

            '' Get axis values
            dim as single leftStickX = GetGamepadAxisMovement(gamepad, GAMEPAD_AXIS_LEFT_X)
            dim as single leftStickY = GetGamepadAxisMovement(gamepad, GAMEPAD_AXIS_LEFT_Y)
            dim as single rightStickX = GetGamepadAxisMovement(gamepad, GAMEPAD_AXIS_RIGHT_X)
            dim as single rightStickY = GetGamepadAxisMovement(gamepad, GAMEPAD_AXIS_RIGHT_Y)
            dim as single leftTrigger = GetGamepadAxisMovement(gamepad, GAMEPAD_AXIS_LEFT_TRIGGER)
            dim as single rightTrigger = GetGamepadAxisMovement(gamepad, GAMEPAD_AXIS_RIGHT_TRIGGER)

            '' Calculate deadzones
            if leftStickX > -leftStickDeadzoneX and leftStickX < leftStickDeadzoneX then leftStickX = 0.0f
            if leftStickY > -leftStickDeadzoneY and leftStickY < leftStickDeadzoneY then leftStickY = 0.0f
            if rightStickX > -rightStickDeadzoneX and rightStickX < rightStickDeadzoneX then rightStickX = 0.0f
            if rightStickY > -rightStickDeadzoneY and rightStickY < rightStickDeadzoneY then rightStickY = 0.0f
            if leftTrigger < leftTriggerDeadzone then leftTrigger = -1.0f
            if rightTrigger < rightTriggerDeadzone then rightTrigger = -1.0f

            if TextFindIndex(TextToLower(GetGamepadName(gamepad)), XBOX_ALIAS_1) > -1 or TextFindIndex(TextToLower(GetGamepadName(gamepad)), XBOX_ALIAS_2) > -1 then
            ...
            elseif TextFindIndex(TextToLower(GetGamepadName(gamepad)), PS_ALIAS) > -1 or TextFindIndex(TextToLower(GetGamepadName(gamepad)), PS4_ALIAS) > -1 then
                ...
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_MIDDLE_LEFT) then DrawCircle(365, 170, 10, RED)
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_MIDDLE) then DrawCircle(405, 170, 10, GREEN)
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_MIDDLE_RIGHT) then DrawCircle(445, 170, 10, BLUE)
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_RIGHT_FACE_LEFT) then DrawCircle(516, 191, 15, GOLD)
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_RIGHT_FACE_DOWN) then DrawCircle(551, 227, 15, BLUE)
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_RIGHT_FACE_RIGHT) then DrawCircle(587, 191, 15, GREEN)
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_RIGHT_FACE_UP) then DrawCircle(551, 155, 15, RED)
                ...
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_LEFT_FACE_UP) then DrawRectangle(247, 147, 24, 29, RED)
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_LEFT_FACE_DOWN) then DrawRectangle(247, 147 + 54, 24, 30, RED)
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_LEFT_FACE_LEFT) then DrawRectangle(217, 176, 30, 25, RED)
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_LEFT_FACE_RIGHT) then DrawRectangle(217 + 54, 176, 30, 25, RED)
                ...
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_LEFT_TRIGGER_1) then DrawRectangleRounded(Rectangle(215, 98, 100, 10), 0.5f, 16, RED)
                if IsGamepadButtonDown(gamepad, GAMEPAD_BUTTON_RIGHT_TRIGGER_1) then DrawRectangleRounded(Rectangle(495, 98, 100, 10), 0.5f, 16, RED)
