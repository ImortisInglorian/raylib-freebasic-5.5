...
dim as zstring * 128 postproShaderText(...) = { _
    "GRAYSCALE", _
    "POSTERIZATION", _
    "DREAM_VISION", _
    "PIXELIZER", _
...
dim as Camera cam
with cam
    .position = Vector3(2.0f, 3.0f, 2.0f)    '' Camera position
...
dim as Model mdl = LoadModel("resources/models/church.obj")                 '' Load OBJ model
dim as Texture2D tex = LoadTexture("resources/models/church_diffuse.png") '' Load model texture (diffuse map)
mdl.materials[0].maps[MATERIAL_MAP_DIFFUSE].texture = tex        '' Set model diffuse texture
...
dim as Shader shaders(MAX_POSTPRO_SHADERS - 1)
...
do while not WindowShouldClose()        '' Detect window close button or ESC key
    ...
    BeginTextureMode(target)       '' Enable drawing to texture
        ...
        BeginMode3D(cam)        '' Begin 3d mode drawing
            DrawModel(mdl, position, 0.1f, WHITE)   '' Draw 3d model with texture
